...
namespace BaiThucHanh4_1
{
    public partial class frmBangPhepCong : Form
    {
            private char code = 'Z';
            private int  n =2;
            private int Row_Start = 10;

            private int Col_Start = 50;
            private int Cell_Width = 30;
            private int Cell_VGap = 5;
            private int Row_Height = 20;

            private int Cell_HGap = 2 ;
        ...
        public frmBangPhepCong(char c, int n0)
        {
            ...
            if (n > 20) TaoLuoi(n);
            else TaoBangPhepToan(n);

        }

        public void TaoBangPhepToan(int n)
        {
            this.Text = "Bảng phép cộng " + MainForm.Ten(code) + n.ToString(); TextBox Cell; this.SuspendLayout();
        ...
        public void TaoLuoi(int n)
        {
            this.Text = "Bảng phép cộng " + MainForm.Ten(code) + n.ToString();

            DataGridView dgv = new DataGridView(); dgv.Location = new System.Drawing.Point(Col_Start, Row_Start); dgv.Name = "dgv";            
            ...
            dgvCol = new DataGridViewTextBoxColumn(); dgvCol.Name = "operator"; dgvCol.HeaderText = "+"; dgvCol.Width = 50; dgv.Columns.Add(dgvCol);

            for (int j = 0; j < n; j++) { dgvCol = new DataGridViewTextBoxColumn(); dgvCol.Name = "Col" + j.ToString("0000"); dgvCol.HeaderText = j.ToString(); dgvCol.Width = 50; dgvCol.FillWeight = 0.01f; dgv.Columns.Add(dgvCol); }

            for (int i = 0; i < n; i++)
            {
                DataGridViewRow dgvRow = new DataGridViewRow();

                DataGridViewTextBoxCell dgvCell = new DataGridViewTextBoxCell(); dgvCell.Value = i.ToString(); dgvRow.Cells.Add(dgvCell);

                for (int j = 0; j < n; j++) { dgvCell = new DataGridViewTextBoxCell(); dgvCell.Value = ((i + j) % n).ToString(); dgvRow.Cells.Add(dgvCell); }
                dgv.Rows.Add(dgvRow);
            }
        }
    }
